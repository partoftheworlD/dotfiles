...
reflector --verbose -n 10 -p http --sort rate --save /etc/pacman.d/mirrorlist --country RU,IT,FI --latest 200 > /dev/null 2>&1
...
pacman -S tmux btop git wget curl ttf-jetbrains-mono inter-font duperemove neovim gamescope lutris steam gamemode less spotify-launcher code gufw tuned cups splix obs-studio obsidian pacman-contrib --noconfirm > /dev/null 2>&1
