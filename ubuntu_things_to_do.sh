...
# Replace Ubuntu Flatpak Repo with Flathub for better package management and apps stability
color_echo "yellow" "Setup Flathub..."
apt install flatpak -y > /dev/null 2>&1
...
apt install tmux btop git curl neovim gamescope lutris steam qbittorrent vlc obs-studio fonts-inter-variable fonts-jetbrains-mono gnome-software gnome-software-plugin-deb gnome-software-plugin-flatpak gnome-software-plugin-fwupd gnome-software-plugin-snap -y > /dev/null 2>&1
flatpak install heroic protonplus org.freedesktop.Platform.ffmpeg-full/x86_64/24.08 -y > /dev/null 2>&1
...
wget -4 -O code.deb https://go.microsoft.com/fwlink/?LinkID=760868  > /dev/null 2>&1
apt install ./code.deb -y > /dev/null 2>&1
...
curl -sS https://download.spotify.com/debian/pubkey_C85668DF69375001.gpg | sudo gpg --dearmor --yes -o /etc/apt/trusted.gpg.d/spotify.gpg > /dev/null 2>&1
echo "deb https://repository.spotify.com stable non-free" | sudo tee /etc/apt/sources.list.d/spotify.list > /dev/null 2>&1
apt update -y > /dev/null 2>&1
apt install spotify-client -y > /dev/null 2>&1
